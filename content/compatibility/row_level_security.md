...
{{< callout type="info" >}}
Row level security is an enterprise feature, therefore you need an enterprise licence to create and alter policies.
{{< /callout >}}
...
All `PERMISSIVE` policies are combined with a boolean 'OR'. There needs to be at least one `PERMISSIVE` policy for the role and command for the current role to be allowed to scan/edit a row. <br>
All `RESTRICTIVE` policies are combined with a boolean 'AND'. If there is no restrictive policy, only the `PERMISSIVE` policies are used. <br>
...
Updates without filters or references to old values, like `UPDATE secrets SET secret = '***';`, will not be validated against a select policy. 
This is also the case for deletes that delete unconditionally, like `DELETE from secrets;`.

For upserts, the table needs to be scanned to decide which new rows will be inserted and which are used to update an existing row.
The select policies are applied after this decision because else it can happen that a row to be updated is filtered and the new row is mistakenly inserted.
...
Usually, select policies silently filter the values. This is not the case on the output values of insert, update and upsert statements where an error is thrown.
...
If the policy is violated when only considering the old rows in the table, an error is thrown if at least one new row violates the insert policy. This leads to the whole statement being rolled back, e.g., 
...
insert into books values (4, 'Alice''s Adventures in Wonderland', 'Lewis Carroll'), (5, 'The Hobbit', 'J. R. R. Tolkien');
----
New row violates row-level security policy for table books;

-- The row with id 4 has also not been added
select * from books;
----
1  Antoine de Saint-Exupéry   The Little Prince
1  Hedwig Munck               The Little King
...
The `using` expression silently filters the rows that may not be updated for update statements. For upserts, an error is thrown in this case.
...
Delete policies filter the values that may be deleted, silently skipping over values violating the policy.
